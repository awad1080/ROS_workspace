...
/**
 * @brief Sets receive "/auxserial" callback function.
 * @param newHumidityCallback "/auxserial" callback function.
 */
...
/**
 * @brief Process "/auxserial" message.
 * @param oscTimeTag OSC time tag associated with message.
 * @param oscMessage Address of OSC message.
 * @return Error code (0 if successful).
 */
...
// Do nothing if no callback assigned
    ...
    //strcpy(ngimuAuxserial.message1,rawData);
    ngimuAuxserial.rawData = rawData;
    oscError = OscMessageGetArgumentAsBlob(oscMessage, &numberOfBytes, rawData, sizeof(rawData));
