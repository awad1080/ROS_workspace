...
void chatterCallback_joystick (const sensor_msgs::Joy::ConstPtr& msg) {
    ...
    double horizontal, vertical, En_button;
    double horizontal_offset, vertical_offset;

    vertical = msg->axes[3]; // Right joystick vertical controls horizontal rudders
    horizontal = msg->axes[2]; // Right joystick horizontal controls vertical rudders
   
    En_button = msg->buttons[4];
    horizontal_offset = offsetValue * horizontal;
    vertical_offset = offsetValue * vertical*-1;

    angles.rudderVertical = vertical_offset + 90;
    angles.rudderHorizontal = horizontal_offset + 90;

    if(En_button){
    if (vertical_offset >= -90 && vertical_offset <= 90) {
        setPWM(4, map(vertical_offset, -90, 90));
        ROS_INFO("vertical_offset angle is %f ", vertical_offset);
    }
    else {
        setPWM(4, map(0, -90, 90));
        ROS_INFO("vertical_offset is %f", vertical_offset);
    }
    if (horizontal_offset >= -90 && horizontal_offset <= 90) {
        setPWM(5, map(horizontal_offset, 90, -90));
        ROS_INFO("horizontal_offset is %f", horizontal_offset);
...
void chatterCallback_joystick(const sensor_msgs::Joy::ConstPtr& msg)
{
	...
	if (move_right_offset >= -90 && move_right_offset <= 90) {
		setPWM(13,map(move_right_offset, -90 ,90)) ;
		ROS_INFO ("move_right_offset angle is %f " ,move_right_offset);
	 }else{
		setPWM(13, map (0,-90,90)) ;
	...
	if(right_top == 1){
		...
		if (move_left_offset >= -90 && move_left_offset <= 90) {
			setPWM(12, map(move_left_offset,-90,90)) ;
			ROS_INFO ("move_left_offset angle is %f " ,move_left_offset);
		} else{
			setPWM(12, map(0 ,-90 ,90)) ;
			ROS_INFO ("move_left_offset angle is %f " ,move_left_offset);
		}
